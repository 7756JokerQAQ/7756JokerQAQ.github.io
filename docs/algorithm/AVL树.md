...
⑥如果一棵二叉树是高度平衡的，他就是AVL树，如果他有n个节点，其高度可保持在搜索的时间复杂度为

⑦AVL树中的任意节点的BF只可能是-1 0 1；

如下图为一颗AVL树：

![](https://cdn.jsdelivr.net/gh/7756JokerQAQ/picodemoo/img/ssss.png)

## AVL的实现

> 实现AVL树的逻辑

1.  首先在`AVLTree`中定义一个节点内部类Node;
2.  构造函数初始化AVL树
3.  基于二分搜索树的实现AVL树
...
**这里可以创建一个`isBalanced()`函数通过递归调用最终的出结果**
...
3.  当插入元素以后更新树的高度，可以利用上面的辅助函数`getHeight()`函数，得到根节点的高度。
...
![](https://cdn.jsdelivr.net/gh/7756JokerQAQ/picodemoo/img/uuu1.png)
...
![](https://cdn.jsdelivr.net/gh/7756JokerQAQ/picodemoo/img/1231.png)

由于红黑树的添加删除操作较为复杂这里值写出2-3树的添加操作如下图（和红黑树完全相同）：

![](https://cdn.jsdelivr.net/gh/7756JokerQAQ/picodemoo/img/ggg.png)
...
2.  初始值均为红色
3.  构建一个判断是否为红色节点的函数
4.  构建左旋转`leftRotate()`右旋转`rightRotate()`和颜色翻转函数`flipColors()`函数
5.  跟据以上准备实现红黑树
...
![](https://cdn.jsdelivr.net/gh/7756JokerQAQ/picodemoo/img/lllkk.png)
...
> **当添加元素的左孩子和右孩子均为红色的时候需要进行颜色的翻转**最终实现的add函数为
